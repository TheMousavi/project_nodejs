...
class commentController extends controller {
    ...
    async destroy(req , res , next){
        ...
        if (! comment) return res.json('دیدگاهی در سیستم ثبت نشده است ');
    ...
    async update(req, res, next){
        ...
        comment.check ? comment.check = false : comment.check = true;
    ...
    async disupdate(req , res , next){
        const comment = await Comment.findById(req.params.id);
        if (! comment) return res.json('دیدگاهی در سیستم ثبت نشده است ');

        //comment.comments.forEach(comment => comment.check = false);


        comment.comments.check = false;
